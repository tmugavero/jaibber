...
export function ProjectInvitePage() {
  ...
  const [invite, setInvite] = useState<ProjectInviteInfo | null>(null);
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        const res = await fetch(`${baseUrl}/api/invites/project/${inviteToken}`);
        if (!res.ok) {
  ...
  const handleAccept = async () => {
    ...
    setError(null);
    try {
      const res = await fetch(`${baseUrl}/api/invites/project/${inviteToken}/accept`, {
        method: "POST",
        ...
        <div className="max-w-md mx-auto pt-8 px-6">
          {invite && (
            <div className="bg-card border border-border rounded-xl p-4 mb-6 text-center">
              ...
              <p className="text-lg font-semibold text-foreground mt-1">{invite.projectName}</p>
              {invite.projectDescription && (
                <p className="text-xs text-muted-foreground mt-1">{invite.projectDescription}</p>
              )}
  ...
  return (
    <div className="min-h-screen bg-background flex items-center justify-center p-6">
      <div className="w-full max-w-sm text-center space-y-6">
        ...
        <div>
          <p className="text-sm text-muted-foreground">You've been invited to join</p>
          <p className="text-2xl font-bold text-foreground mt-2">{invite?.projectName}</p>
          {invite?.projectDescription && (
            <p className="text-sm text-muted-foreground mt-2">{invite.projectDescription}</p>
          )}
